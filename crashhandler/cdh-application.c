...
static CdmStatus init_crashdump_archive(CdhApplication *app, const gchar *dirname)
{
    ...
    aname = g_strdup_printf(
        ARCHIVE_NAME_PATTERN, dirname, app->context->name, app->context->pid, app->context->tstamp);
...
static void do_crash_actions(CdmOptions *options, const gchar *proc_name, gboolean postcore)
{
...
enter_cleanup:
    ...
    if (cdh_manager_connected(app->manager)) {
        ...
        type = (status == CDM_STATUS_OK ? CDM_MESSAGE_COREDUMP_SUCCESS
