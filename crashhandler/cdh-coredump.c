...
static CdmStatus get_nt_file_region(CdhCoredump *cd,
                                    Elf64_Addr address,
                                    Elf64_Addr *region_start,
                                    Elf64_Addr *region_end,
                                    Elf64_Off *file_start,
...
static CdmStatus read_notes(CdhCoredump *cd)
{
    ...
    if (cdh_archive_stream_read(
            cd->archive, cd->context->nhdr, cd->context->pphdr[prog_note].p_filesz)
        != CDM_STATUS_OK) {
...
static CdmStatus init_coredump(CdhCoredump *cd)
{
    ...
    if (cdh_archive_stream_open(
            cd->archive, 0, (dst != NULL ? dst : "coredump"), CDM_CRASHDUMP_SPLIT_SIZE)
        == CDM_STATUS_OK) {
...
CdmStatus cdh_coredump_generate(CdhCoredump *cd)
{
    ...
    if (read_elf_headers(cd) == CDM_STATUS_OK) {
        ...
        if (phdr == -1) {
...
#ifdef __x86_64__
            cd->context->ra
                = read_virtual_memory(cd, cd->context->regs.rbp + return_addr_add, phdr);
#elif __aarch64__
            ...
            region_found = get_nt_file_region(
                cd, cd->context->ra, &region_start, &region_end, &region_file_offset, &region_name);

            if (region_found == CDM_STATUS_ERROR)
                g_info("Could not get NT_FILE region of the return address");
            else {
                cd->context->ra_file_offset
                    = cd->context->ra - region_start
