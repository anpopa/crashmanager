  ...
  if (ctx->crashid_info & CID_IP_FILE_OFFSET)
    {
      if (ctx->crashid_info & CID_RA_FILE_OFFSET)
        {
          cid_str = g_strdup_printf ("%s%lx%s%s", ctx->name, ctx->ip_file_offset,
                                     ctx->ip_module_name, ctx->ra_module_name);
        }
      else
        {
          cid_str
              = g_strdup_printf ("%s%lx%s", ctx->name, ctx->ip_file_offset, ctx->ip_module_name);
        }
    }
  else
    {
...
#ifdef __x86_64__
  g_info ("Crash in %s contextID=%s process=\"%s\" thread=\"%s\" pid=%ld cpid=%ld crashID=%s "
          "vectorID=%s "
          "confidence=\"%s\" signal=\"%s\" rip=0x%lx rbp=0x%lx retaddr=0x%lx IPFileOffset=0x%lx "
          "RAFileOffset=0x%lx IPModule=\"%s\" RAModule=\"%s\"",
          loc, ctx->contextid, ctx->name, ctx->tname, ctx->pid, ctx->cpid, ctx->crashid,
          ctx->vectorid, CRASH_ID_QUALITY (ctx->crashid_info), strsignal ((gint)ctx->sig),
          ctx->regs.rip, ctx->regs.rbp, ctx->ra, ctx->ip_file_offset, ctx->ra_file_offset,
          ctx->ip_module_name, ctx->ra_module_name);
#elif __aarch64__
  g_info ("Crash in %s contextID=%s process=\"%s\" thread=\"%s\" pid=%ld cpid=%ld crashID=%s "
          "vectorID=%s "
          ...
          "RAFileOffset=0x%lx IPModule=\"%s\" RAModule=\"%s\"",
          loc, ctx->contextid, ctx->name, ctx->tname, ctx->pid, ctx->cpid, ctx->crashid,
          ctx->vectorid, CRASH_ID_QUALITY (ctx->crashid_info), strsignal ((int)ctx->sig),
          ctx->regs.pc, ctx->regs.lr, ctx->ra, ctx->ip_file_offset, ctx->ra_file_offset,
          ctx->ip_module_name, ctx->ra_module_name);
#endif
  ...
  while ((nfile = g_dir_read_name (gdir)) != NULL)
    {
      ...
      fline = g_strdup_printf ("%s  %u  %u %u %ld %4s %s", mbuf, (unsigned int)fstat.st_nlink,
                               fstat.st_uid, fstat.st_gid, fstat.st_size, nfile, fmode);
...
cdh_context_generate_poststream (CdhContext *ctx)
{
  ...
  file_data = g_strdup_printf (
      ...
      "RAModuleName   = %s\n"
      "CoredumpSize   = %lu\n",
      ctx->name, ctx->tname, ctx->pexe,
      ctx->lifecycle_state != NULL ? ctx->lifecycle_state : "unavailable", ctx->tstamp, ctx->pid,
      ctx->cpid, ctx->sig, ctx->crashid, ctx->vectorid, ctx->contextid,
      ctx->context_name != NULL ? ctx->context_name : "unavailable", ip, ra, ctx->ip_file_offset,
      ctx->ra_file_offset, ctx->ip_module_name, ctx->ra_module_name, ctx->cdsize);
  ...
  if ((status == CDM_STATUS_OK) && (ctx->epilog != NULL))
    {
      if (cdh_archive_create_file (ctx->archive, "info.epilog", strlen (ctx->epilog))
          == CDM_STATUS_OK)
        {
          status = cdh_archive_write_file (ctx->archive, (const void *)ctx->epilog,
                                           strlen (ctx->epilog));
