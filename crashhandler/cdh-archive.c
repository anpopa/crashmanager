...
CdmStatus cdh_archive_create_file(CdhArchive *ar, const gchar *dst, gsize file_size)
{
    g_assert(ar);
    g_assert(dst);
...
CdmStatus cdh_archive_stream_read_all(CdhArchive *ar, gboolean dummy_write)
{
    g_assert(ar);
    g_assert(ar->in_stream);
