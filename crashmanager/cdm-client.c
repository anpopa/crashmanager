...
static void send_context_info(CdmClient *c, const gchar *context_name)
{
...
static void send_epilog(CdmClient *c, CdmJournalEpilog *elog)
{
    ...
    if (frame_cnt > 0) {
        for (guint64 i = 0; i < frame_cnt; i++) {
            ...
            cdm_message_set_epilog_frame_data(
                fmsg, elog->backtrace + (i * CDM_MESSAGE_EPILOG_FRAME_MAX_LEN));
