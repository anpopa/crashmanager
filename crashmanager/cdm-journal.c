...
CdmJournal *cdm_journal_new(CdmOptions *options, GError **error)
{
    ...
    if (sqlite3_open(opt_dbpath, &journal->database)) {
        ...
        if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error)
            ...
            g_set_error(
                error, g_quark_from_static_string("JournalNew"), 1, "Create crash table fail");
        }
    ...
    g_source_set_callback(
        journal->source, G_SOURCE_FUNC(source_timer_callback), journal, source_destroy_notify);
                              ...
                              GError **error)
{
    ...
    if (file_size == -1) {
        g_set_error(
            error, g_quark_from_static_string("JournalAddCrash"), 1, "Cannot stat file for size");
        return 0;
    }
                              ...
                              GError **error)
{
    ...
    if (!file_path) {
        g_set_error(
            error, g_quark_from_static_string("JournalSetTransfer"), 1, "Invalid arguments");
    } else {
        g_autofree gchar *sql = NULL;
        gchar *query_error = NULL;
        ...
        sql = g_strdup_printf(
            "UPDATE %s SET TSTATE = %d WHERE ID IS %lu", cdm_journal_table_name, complete, id);

        if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error)
            != SQLITE_OK) {
            g_set_error(
                error, g_quark_from_static_string("JournalSetTransfer"), 1, "SQL query error");
                             ...
                             GError **error)
{
    ...
    if (!file_path) {
        ...
        sql = g_strdup_printf(
            "UPDATE %s SET RSTATE = %d WHERE ID IS %lu", cdm_journal_table_name, complete, id);

        if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error)
            != SQLITE_OK) {
            g_set_error(
                error, g_quark_from_static_string("JournalSetRemoved"), 1, "SQL query error");
...
gchar *cdm_journal_get_untransferred(CdmJournal *journal, GError **error)
{
    ...
    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetUntrasnferred"), 1, "SQL query error");
