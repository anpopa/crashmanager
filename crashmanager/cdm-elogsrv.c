 ...
 */
static gboolean elogsrv_source_dispatch(GSource *source, GSourceFunc callback, gpointer cdmelogsrv);
...
CdmELogSrv *cdm_elogsrv_new(CdmOptions *options, CdmJournal *journal, GError **error)
{
    ...
    if (elogsrv->sockfd < 0) {
        ...
        g_set_error(
            error, g_quark_from_static_string("ServerNew"), 1, "Fail to create elogsrv socket");
    } else {
        ...
        if (setsockopt(elogsrv->sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *) &tout, sizeof(tout))
            == -1)
            g_warning("Failed to set the epilog socket receiving timeout: %s", strerror(errno));

        if (setsockopt(elogsrv->sockfd, SOL_SOCKET, SO_SNDTIMEO, (char *) &tout, sizeof(tout))
            == -1)
...
CdmStatus cdm_elogsrv_bind_and_listen(CdmELogSrv *elogsrv)
{
    ...
    if (status == CDM_STATUS_ERROR) {
        ...
        elogsrv->tag
            = g_source_add_unix_fd(CDM_EVENT_SOURCE(elogsrv), elogsrv->sockfd, G_IO_IN | G_IO_PRI);
    }
