...
static void wait_early_cdh_instances(long timeout)
{
...
static CdmStatus archive_early_crashdump(CdmApplication *app, const gchar *crashfile)
{
...
static CdmStatus archive_early_crashes(CdmApplication *app, const gchar *crashdir)
{
    CdmStatus status = CDM_STATUS_OK;
...
static CdmStatus archive_kdumps(CdmApplication *app, const gchar *crashdir)
{
    ...
    if (error != NULL) {
        ...
        while ((nfile = g_dir_read_name(gdir)) != NULL) {
            ...
            if (g_rename(fpath, entry_path) != 0) {
                g_warning(
                    "Fail to move kdump %s to %s. Error %s", fpath, entry_path, strerror(errno));
                continue;
            }
        }
...
static void transfer_complete(gpointer cdmjournal, const gchar *file_path)
{
    CDM_UNUSED(cdmjournal);
    g_info("Archive transfer complete for %s", file_path);
}

static void transfer_missing_files(CdmApplication *app)
{
